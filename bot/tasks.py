...
async def request_milk(bot: Bot, scheduler, chat_id: Optional[int] = None, message_thread_id: Optional[int] = None) -> None:
    ...
    now = datetime.now()
    scheduler.add_job(remind_if_needed, "date", run_date=now + timedelta(minutes=REMIND_AFTER_MIN), args=[bot, "milk"])
    scheduler.add_job(escalate_if_needed, "date", run_date=now + timedelta(minutes=ESCALATE_AFTER_MIN), args=[bot, "milk"])
...
async def request_bakery(bot: Bot, scheduler, chat_id: Optional[int] = None, message_thread_id: Optional[int] = None) -> None:
    ...
    now = datetime.now()
    scheduler.add_job(remind_if_needed, "date", run_date=now + timedelta(minutes=REMIND_AFTER_MIN), args=[bot, "bakery"])
    scheduler.add_job(escalate_if_needed, "date", run_date=now + timedelta(minutes=ESCALATE_AFTER_MIN), args=[bot, "bakery"])
...
async def request_freezer(bot: Bot, scheduler, chat_id: Optional[int] = None, message_thread_id: Optional[int] = None) -> None:
    ...
    now = datetime.now()
    scheduler.add_job(remind_if_needed, "date", run_date=now + timedelta(minutes=REMIND_AFTER_MIN), args=[bot, "freezer"])
    scheduler.add_job(escalate_if_needed, "date", run_date=now + timedelta(minutes=ESCALATE_AFTER_MIN), args=[bot, "freezer"])
...
async def request_opening(bot: Bot, scheduler, chat_id: int = None) -> None:
    ...
    now = datetime.now()
    scheduler.add_job(remind_if_needed, "date", run_date=now + timedelta(minutes=REMIND_AFTER_MIN), args=[bot, "opening"])
    scheduler.add_job(escalate_if_needed, "date", run_date=now + timedelta(minutes=ESCALATE_AFTER_MIN), args=[bot, "opening"])
...
async def request_cash(bot: Bot, scheduler, chat_id: int = None) -> None:
    ...
    now = datetime.now()
    scheduler.add_job(remind_if_needed, "date", run_date=now + timedelta(minutes=REMIND_AFTER_MIN), args=[bot, "cash"])
    scheduler.add_job(escalate_if_needed, "date", run_date=now + timedelta(minutes=ESCALATE_AFTER_MIN), args=[bot, "cash"])
...
async def request_closing(bot: Bot, scheduler, chat_id: int = None) -> None:
    ...
    now = datetime.now()
    scheduler.add_job(remind_if_needed, "date", run_date=now + timedelta(minutes=REMIND_AFTER_MIN), args=[bot, "closing"])
    scheduler.add_job(escalate_if_needed, "date", run_date=now + timedelta(minutes=ESCALATE_AFTER_MIN), args=[bot, "closing"])
